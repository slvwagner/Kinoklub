...
```{r echo=FALSE}
if(df_keine_Rechnnung$`keine Verleiherrechnung`[ii]) {
  writeLines("Achtung es gibt noch keine Verleiherabrechnung für diesen Film. Deshalb fehlen die Reklame- und Porto-Kosten.\nDer Gewinn kann sich deshalb noch ändern.")
}
...
df_temp|>
  select(`Gewinn/Verlust [CHF]`)|>
  rename(`Film Gewinn [CHF]` = `Gewinn/Verlust [CHF]`)|>
  knitr::kable()
...
df_GV_Eintritt|>
  filter(Datum == c_Datum & `Suisa Nummer` == c_suisa)|>
  select(`Sonstige Kosten`)|>
  rename(`Reklamematerial und Porto [CHF]` = `Sonstige Kosten`)|>
  knitr::kable()
```
