...
Eventeinnahmen <- Einnahmen_und_Ausgaben[["Einnahmen"]]|>
  filter(Kategorie == "Vermietung", Datum == c_Datum)|>
...
Eventausgaben <- Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == "Eventausgaben", Spieldatum == c_Datum)|>
  select(Bezeichnung, Betrag)|>
  rename(`Betrag [CHF]` = Betrag)
...
### Umsatz
Verkaufsartikel ohne Lieferant sind Spezialpreise definiert.
```{r echo=FALSE}
df_temp|>
  filter(is.na(Lieferant))|>
...
Kiosk <- df_Kiosk|>
  filter(Datum == c_Datum)|>
  reframe(`Kiosk Gewinn [CHF]` = sum(Gewinn, na.rm = T)|>round5Rappen()
          )
