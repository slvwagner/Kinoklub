---
title: "Jahresabrechnung Kinoklub"
output: 
  html_document:
    css: Kinoklub_dark.css
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
act_date <- Sys.time()|>as.Date()
act_date <- paste0(lubridate::day(act_date),".", lubridate::month(act_date),".", lubridate::year(act_date))
```

Bericht wurde am `r act_date` erstellt.

## Kino Vorführungen
```{r echo=FALSE}
if(sum(df_keine_Rechnnung$`keine Verleiherrechnung`)>0) {
  writeLines("Achtung es sind noch nicht alle Filmabrechnungen vorhanden. Die Nebenkosten können sich noch ändern und demnach die die MWST auf die Verleiherabgaben")
}
```


```{r}
df_temp <- bind_rows(
  tibble(Bezeichnung = "Einnahmen Ticketverkauf",
         df_Eintritt |>
           reframe(`Betrag [CHF]` = sum(Umsatz))),
  tibble(
    Bezeichnung = "Suisaabgaben",
    df_GV_Eintritt |>
      select(Datum, Filmtitel, `SUISA-Abzug [CHF]`) |>
      reframe(`Betrag [CHF]` = -sum(`SUISA-Abzug [CHF]`))),
  tibble(
    Bezeichnung = "Verleigerabgaben",
    df_GV_Eintritt |>
      select(Datum, Filmtitel, `Verleiher-Abzug [CHF]`) |>
      reframe(`Betrag [CHF]` = -sum(`Verleiher-Abzug [CHF]`))),
  tibble(
    Bezeichnung = "Nebenkosten Verleigerabgaben",
    df_GV_Eintritt |>
      select(Datum, Filmtitel, `Sonstige Kosten [CHF]`) |>
      reframe(`Betrag [CHF]` = -sum(`Sonstige Kosten [CHF]`, na.rm = T))),
  tibble(Bezeichnung = "MWST auf Verleiherleistungen",
         df_GV_Eintritt|>
           select(`MWST auf die Verleiherrechnung [CHF]`)|>
           reframe(`Betrag [CHF]` = -sum(`MWST auf die Verleiherrechnung [CHF]`))
         )
)  

df_temp|>
  knitr::kable()
```


```{r}
df_temp|>
  reframe(`Summe [CHF]`=sum(`Betrag [CHF]`))|>
  knitr::kable()
```


## Eventausgaben

```{r}
Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == "Eventausgaben")|>
  select(Bezeichnung, Betrag)|>
  reframe(`Summe [CHF]` = sum(Betrag,na.rm = T))|>
  knitr::kable()
```

## Kiosk

```{r}
df_temp <- bind_rows(
  tibble(
    Bezeichnung = "Einnahmen aus Verkauf",
    df_Kiosk |>
      reframe(`Betrag [CHF]` = sum(Kassiert, na.rm = T)) |>
      arrange(desc(`Betrag [CHF]`))
  ),
  tibble(
    Bezeichnung = "Einkauf",
    Einnahmen_und_Ausgaben[["Ausgaben"]] |>
      filter(Kategorie == "Kiosk") |>
      reframe(`Betrag [CHF]` = -sum(Betrag))
  )
)

df_temp|>
  knitr::kable()
```

```{r}
df_temp|>
  reframe(`Summe [CHF]` = sum(`Betrag [CHF]`, na.rm = T))|>
  knitr::kable()

```

## Kassa Überschuss / Manko 
**Achtung!** \
Die Kioskkasse wird auch für Barauszahlung von stornierten Tickets genutzt. 

```{r}
df_manko_uerberschuss|>
  select(-Datum)|>
  reframe(`Überschuss / Manko` = sum(`Überschuss / Manko`, na.rm = T))|>
  rename(`Überschuss / Manko [CHF]` = `Überschuss / Manko`)|>
  knitr::kable()
```

## Werbung

```{r echo=FALSE}
df_temp <- bind_rows(
  tibble(
    Bezeichnung = "Kinowerbeeinnahmen",
    Einnahmen_und_Ausgaben[["Einnahmen"]] |>
      filter(Kategorie == "Werbung") |>
      reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T))
  ),
  tibble(
    Bezeichnung = "Werbeausgaben",
    Einnahmen_und_Ausgaben[["Ausgaben"]] |>
      filter(Kategorie == "Werbung") |>
      reframe(`Betrag [CHF]` = -sum(Betrag, na.rm = T))
  )
)

df_temp|>
  knitr::kable()
```

```{r}
df_temp|>
  reframe(`Summe [CHF]` = sum(`Betrag [CHF]`))|>
  knitr::kable()

```


## Vermietungen 

```{r echo=FALSE}
df_temp <- bind_rows(
    tibble(Bezeichnung = "Kinovermietungseinnahmen",
           Einnahmen_und_Ausgaben[["Einnahmen"]] |>
             filter(Kategorie == "Vermietung") |>
             reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T))),
    tibble(Bezeichnung = "Vermietungsausgaben",
           Einnahmen_und_Ausgaben[["Ausgaben"]] |>
             filter(Kategorie == "Vermietung") |>
             reframe(`Betrag [CHF]` = -sum(Betrag, na.rm = T)))
)

df_temp|>
  knitr::kable()

```
```{r}
df_temp|>
  reframe(`Summe [CHF]` = sum(`Betrag [CHF]`, na.rm = T))|>
  knitr::kable()

```

## Personalaufwand
```{r echo=FALSE}
Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == "Personalaufwand")|>
  select(Datum, Bezeichnung,Betrag)|>
  group_by(Bezeichnung)|>
  reframe(Betrag = sum(Betrag, na.rm = T))|>
  rename(`Betrag [CHF]` = Betrag)|>
  knitr::kable()

```

```{r echo=FALSE}
Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == "Personalaufwand")|>
  reframe(`Summe [CHF]` = sum(Betrag))|>
  knitr::kable()

```


## Sonstiges

### Einnahmen
```{r echo=FALSE}
Einnahmen_und_Ausgaben[["Einnahmen"]]|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4])|>
  select(Datum, Bezeichnung,Betrag)|>
  rename(`Betrag [CHF]` = Betrag)|>
  arrange(Datum)|>   
  mutate(Datum = paste0(day(Datum),".",month(Datum),".",year(Datum)))|>
  knitr::kable()

```

```{r echo=FALSE}
Einnahmen_und_Ausgaben[["Einnahmen"]]|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4])|>
  reframe(`Summe [CHF]` = sum(Betrag, na.rm = T))|>
  knitr::kable()

```

### Ausgaben
```{r echo=FALSE}
Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4])|>
  select(Datum, Bezeichnung,Betrag)|>
  group_by(Bezeichnung)|>
  reframe(Betrag = sum(Betrag, na.rm = T))|>
  rename(`Betrag [CHF]` = Betrag)|>
  # arrange(Datum)|>
  # mutate(Datum = paste0(day(Datum),".",month(Datum),".",year(Datum)))|>
  knitr::kable()

```

```{r echo=FALSE}
Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4])|>
  reframe(`Summe [CHF]` = sum(Betrag))|>
  knitr::kable()

```

### Differenz
```{r}
(Einnahmen_und_Ausgaben[["Einnahmen"]]|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4])|>
  reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T))-
  Einnahmen_und_Ausgaben[["Ausgaben"]]|>
  filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4])|>
  reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T)))|>
  knitr::kable()

```



## Gewinn / Verlust
```{r echo=FALSE}
if(sum(df_keine_Rechnnung$`keine Verleiherrechnung`)>0) {
  writeLines("Achtung es sind noch nicht alle Filmabrechnungen vorhanden. Die Einnahmen aus dem Ticketverkauf werden sich noch ändern.")
}
```

```{r}
df_temp <- tibble(
  Kategorie = c("Ticketverkauf", "Eventausgaben","Kiosk", "Kasse: Überschuss / Manko ","Werbung", "Vermietung", "Personalaufwand","Sonstiges"),
  `Betrag [CHF]` = c(
    df_GV_Eintritt |>
      select(Datum, Filmtitel, `Gewinn/Verlust [CHF]`) |>
      reframe(`Betrag [CHF]` = sum(`Gewinn/Verlust [CHF]`)) |>
      pull(),
    (
        Einnahmen_und_Ausgaben[["Ausgaben"]]|>
          filter(Kategorie == "Eventausgaben")|>
          select(Bezeichnung, Betrag)|>
          reframe(`Summe [CHF]` = sum(Betrag,na.rm = T))
    )|>
      pull(),
    (
      df_Kiosk |>
        reframe(`Betrag [CHF]` = sum(Kassiert, na.rm = T)) |>
        arrange(desc(`Betrag [CHF]`)) -
        Einnahmen_und_Ausgaben[["Ausgaben"]] |>
        filter(Kategorie == "Kiosk") |>
        reframe(`Betrag [CHF]` = sum(Betrag))
    ) |>
      pull(),
    (
      df_manko_uerberschuss|>
        select(-Datum)|>
        reframe(`Überschuss / Manko [CHF]` = sum(`Überschuss / Manko`, na.rm = T))
    )|>
      pull(),
    (
      Einnahmen_und_Ausgaben[["Einnahmen"]] |>
        filter(Kategorie == "Werbung") |>
        reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T)) -
        Einnahmen_und_Ausgaben[["Ausgaben"]] |>
        filter(Kategorie == "Werbung") |>
        reframe(`Betrag [CHF]` = sum(Betrag))
    ) |>
      pull(),
    (
      Einnahmen_und_Ausgaben[["Einnahmen"]] |>
        filter(Kategorie == "Vermietung") |>
        reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T)) -
        Einnahmen_und_Ausgaben[["Ausgaben"]] |>
        filter(Kategorie == "Vermietung") |>
        reframe(`Betrag [CHF]` = sum(Betrag))
    ) |>
      pull(),
    -(Einnahmen_und_Ausgaben[["Ausgaben"]] |>
      filter(Kategorie == "Personalaufwand") |>
      reframe(`Summe [CHF]` = sum(Betrag, na.rm = T)) |>
      pull()), 
    (
      Einnahmen_und_Ausgaben[["Einnahmen"]] |>
        filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4]) |>
        reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T)) -
        Einnahmen_und_Ausgaben[["Ausgaben"]] |>
        filter(Kategorie == Einnahmen_und_Ausgaben$dropdown$`drop down`[4]) |>
        reframe(`Betrag [CHF]` = sum(Betrag, na.rm = T))
    ) |>
      pull()
  )
) 

df_temp|>
  knitr::kable()
```

```{r}
df_temp|>
  reframe("Gewinn / Verlust [CHF]" = sum(`Betrag [CHF]`))|>
  knitr::kable()
```


\
\
\
\
\
\
