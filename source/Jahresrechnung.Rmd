...
df_temp1 <- Einnahmen_und_Ausgaben$Ausgaben|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[6])|>
  arrange(desc(Datum))|>
...
df_temp <- Einnahmen_und_Ausgaben$Einnahmen|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  select(-Datum)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[2])
...
KioskEinnahmen <- df_Kiosk|>
  ...
  reframe(Anzahl = sum(Anzahl),
          `Betrag [CHF]` = sum(`Kassiert [CHF]`, na.rm = T))|>
...
EinkaufGetränke <- df_Kiosk|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Lieferant == l_data$Lieferant$Lieferantenname[2])|>
...
EinkaufKino <- Einnahmen_und_Ausgaben$Ausgaben|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[3])|>
  arrange(Datum)|>   
  mutate(Datum = paste0(day(Datum),".",month(Datum),".",year(Datum)))|>
...
df_temp <- df_manko_uerberschuss|>
  left_join(l_data$Programm, by = join_by(`Event ID`))|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  mutate(Datum_display = format(Datum, "%d.%m.%Y"))|>
  select(`Event ID`, Datum_display, Datum, Zeit, Filmtitel, `Überschuss / Manko`)
df_temp
```
...
```{r}
Kasse <- df_temp|>
  reframe(`Überschuss / Manko [CHF]` = sum(`Überschuss / Manko`, na.rm = T))
...
df_temp <- Einnahmen_und_Ausgaben$Einnahmen|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[8])
...
df_temp1 <- Einnahmen_und_Ausgaben$Ausgaben|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[7])
...
df_temp <- Einnahmen_und_Ausgaben$Ausgaben|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[4])|>
  select(Datum, Bezeichnung,`Betrag [CHF]`)|>
  arrange(Datum)|>
  mutate(Datum = paste0(day(Datum),".",month(Datum),".",year(Datum)))
...
df_temp <- Einnahmen_und_Ausgaben$Einnahmen|>
  filter(lubridate::year(Datum) == Abrechungsjahr)|>
  filter(Kategorie == l_data$Kategorie$Auswahl[5])
