...
df_temp <- df_Abrechnung |>
  ...
  select(Datum, Datum_display, Anfang, Suisanummer, Filmtitel, `Gewinn/Verlust Filmvorführungen [CHF]`)
```
...
df_temp <- df_Abrechnung|>
  ...
  select(Datum, Datum_display,Anfang, Suisanummer, Filmtitel, `Umsatz [CHF]`, `Gewinn/Verlust Tickets [CHF]`)
...
```{r echo=FALSE}
df_temp|> 
  select(Datum, Datum_display, Anfang,`Suisanummer`, Filmtitel, Besucher, `Umsatz [CHF]`, `Umsatz pro Gast [CHF]`)|>
    DT::datatable(
    df_temp,
    options = list(columnDefs = list(
      # list(targets = 3,visible = F),
...
if(nrow(df_temp)>1){
  ...
  m <- str_split(timeRange, "-")|>
    sapply(function(range){
      lubridate::hm(df_temp$Anfang) >= lubridate::hm(range[1]) & lubridate::hm(df_temp$Anfang) < lubridate::hm(range[2])
    })
  colnames(m) <- timeRange
  m
...
df_temp <- m |>
  ...
  group_by(weekday, `Time Range`) |>
  reframe(
    d_gewinn = round5Rappen(sum(`Umsatz pro Gast [CHF]`) / n())
  ) |>
  mutate(
