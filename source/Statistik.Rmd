...
df_temp <- df_temp|>
  ...
  select(Datum_display, Datum,`Event ID`, Artikelname, `Verkaufspreis [CHF]`, Anzahl, `Umsatz [CHF]`)
...
```{r }
df_temp|>
  reframe(`Durchschnittlicher Umsatz pro Gast [CHF]` = 
            sum(`Umsatz [CHF]`, na.rm = T) / sum(Anzahl, na.rm = T)
          )|>
...
```{r}
df_temp|>
  group_by(Suisanummer, Filmtitel)|>
...
df_temp|>
  ggplot()+
  geom_col(aes(x = Datum, y = `Umsatz [CHF]`, fill = as.factor(Zahlend)),
           position = "jitter",
           show.legend =  FALSE)+
