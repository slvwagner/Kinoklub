...
df_Eintritt|>
  ...
  group_by(Platzkategorie, Verkaufspreis, Zahlend)|>
  reframe(Anzahl = sum(Anzahl))|>
  arrange((Zahlend), (Anzahl))|> # arrange the plot
  mutate(Platzkategorie = factor(Platzkategorie)|>fct_inorder(),
...
df_temp <- df_Eintritt|>
  ...
  mutate(Platzkategorie = factor(Platzkategorie)|>fct_inorder(),
         Verkaufspreis = factor(Verkaufspreis))

df_temp|>
  ggplot(aes(x = Platzkategorie, `Umsatz [CHF]`, fill = Verkaufspreis))+
  geom_col()+
...
<!-- ```{r fig.width=10} -->
<!-- df_temp|> -->
<!--   ggplot(aes(Platzkategorie, Umsatz, fill = `Verkaufspreis [CHF]`))+ -->
<!--   geom_col()+ -->
<!--   coord_flip()+ -->
<!--   my_template+ -->
