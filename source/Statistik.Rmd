...
```{r echo=FALSE}
df_temp <- df_Abrechnung|>
  select(Datum, Zeit, Filmtitel, `Umsatz [CHF]`, `Suisavorabzug [CHF]`,`Verleiherrechnungsbetrag [CHF]`, `MWST [CHF]`)|>
  arrange(desc(Datum))
```
...
df_temp <- df_Kiosk|>
  ...
  reframe(Anzahl = sum(Anzahl), 
          ...
          `Gewinn [CHF]` = sum(`Gewinn [CHF]`)|>round(2)
          )|>
  arrange(desc(Anzahl))|>
...
df_temp <- df_Kiosk|>
  ...
  select(Datum_display, Datum,`Event ID`, Verkaufsartikel, `Verkaufspreis [CHF]`, Anzahl, `Umsatz [CHF]`)
...
df_temp <- df_Kiosk|>
  filter(lubridate::year(Datum) == c_Abrechnungsjahr)|>
  mutate(Datum_display = format(Datum, "%d.%m.%Y"))|>
  select(`Event ID`, Verkaufsartikel, `Verkaufspreis [CHF]`, Anzahl, `Umsatz [CHF]`)
```
...
```{r fig.width=10}
df_temp|>
  group_by(`Event ID`, Datum)|>
  reframe(`Umsatz [CHF]` = sum(`Umsatz [CHF]`,na.rm = T))|>
  ggplot()+
  geom_col(aes(x = Datum, y = `Umsatz [CHF]`, fill = as.factor(`Event ID`)),
           color = "black",
           show.legend =  FALSE)+
