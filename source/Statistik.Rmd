...
```{r}
df_temp2 <- df_temp2|>
  reframe(`Summe der Anzahl` = sum(Anzahl, na.rm = T),
          `Summe [CHF]` = sum(Umsatz, na.rm = T))
...
### Kredit 
Die Differenz von Einnahmen und Eingelöststen Abos. 

```{r}
tibble(`Differenz [CHF]` = df_temp$`Summe Betrag [CHF]` - df_temp2$`Summe [CHF]`)|>
  knitr::kable()
```
