...
```{r}
df_temp <- df_Abrechnung|>
  # select(Datum, Zeit, `Suisa Nummer`, Filmtitel, Brutto, )|>
...
if(df_temp$`Gewinn`|>mean(na.rm = TRUE) > df_temp$`Gewinn`|>median(na.rm = TRUE)){
  gewinn <- df_temp$`Gewinn`|>median(na.rm = TRUE)
}else{
  gewinn <- df_temp$`Gewinn`|>mean(na.rm = TRUE)
}
...
df_temp <- df_Kiosk|>
  ...
  select(Datum_display, Datum,`Event ID`, Verkaufsartikel, `Verkaufspreis [CHF]`, Anzahl, `Kassiert [CHF]`)
...
```{r }
df_temp1 <- df_temp|>
  reframe(`Durchschnittlicher Umsatz pro Gast [CHF]` = sum(`Kassiert [CHF]`, na.rm = T)/sum(Besucher, na.rm = T))
df_temp1|>
...
df_temp <- df_Kiosk|>
  mutate(Datum_display = format(Datum, "%d.%m.%Y"))|>
  select(`Event ID`, Verkaufsartikel, `Verkaufspreis [CHF]`, Anzahl, `Kassiert [CHF]`)|>
  rename(`Umsatz [CHF]` = `Kassiert [CHF]`)
```
