...
df_Abrechnung$Zeit|>format("h%:m%")

df_temp|>
  select(Datum, Datum_display, Zeit, Suisanummer, Filmtitel, `Gewinn/Verlust Filmvorführungen [CHF]`)
```
...
df_temp <- df_Abrechnung|>
  ...
  select(Datum, Datum_display,Zeit, Suisanummer, Filmtitel, `Umsatz [CHF]`, `Gewinn/Verlust Tickets [CHF]`)
...
```{r }
df_temp <- df_Abrechnung|>
  select(Datum, Zeit, Filmtitel, `Umsatz [CHF]`, `Gewinn/Verlust Tickets [CHF]`)|>
  arrange(Datum)
df_temp <- df_temp|>
  arrange(Datum)|>
...
```{r echo=FALSE}
df_temp <- df_Abrechnung|>
  select(Datum, Zeit, Filmtitel, `Umsatz [CHF]`, `SUISA-Vorabzug [CHF]`,`Verleiherabzug [CHF]`, `MWST [CHF]`)|>
  arrange(desc(Datum))
```
...
df_temp <- df_Abrechnung|>
  ...
  select(Datum, Datum_display, Zeit, Filmtitel, `Umsatz [CHF]`, `SUISA-Vorabzug [CHF]`,`MWST [CHF]`,`Verleiherabzug [CHF]`)|>
  arrange(desc(Datum))
```
