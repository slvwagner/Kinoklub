...
```{r}
df_temp1 <- left_join(df_temp1,
          Programm|>
            select(-Datum),
          by = c(`Event ID` = "Event ID")
          )
```
...
df_temp1 <- df_temp1|>
  ...
  select(ID,`Event ID`,Datum, Abrechnungsjahr,Datum_display,Bezeichnung, `Betrag [CHF]`)|>
...
df_temp <- l_abrechnung|>   
  ...
  mutate(Datum_display = format(Datum, "%d.%m.%Y"),
         ...
         `Suisavorabzug [CHF]` = round(`Suisavorabzug [CHF]`,2),
        )|>
...
```{r}
KioskEinnahmen <- df_temp|>
  group_by(Artikelname, `Einzelpreis [CHF]`)|>
  reframe(Anzahl = sum(Anzahl),
          `Betrag [CHF]` = sum(`Betrag [CHF]`, na.rm = T))|>
...
EinkaufGetränke <- df_Kiosk|>
  ...
  group_by(`Artikelname`,`Einzelpreis [CHF]`, `Einkaufspreis [CHF]`)|>
  reframe(Anzahl = sum(Anzahl),
...
Miete <- Ausgaben|>
  ...
  select(-`Event ID`, -Referenz, -Adresse, -Rechnungsnummer, -Buchungskonto)|>
  mutate(Datum = format(Datum, "%d.%m.%Y"))
```
