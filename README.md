...
Wenn Probleme auftreten oder Änderungen notwendig sind, können diese direkt auf GitHub als Issue erfasst werden:
...
2.  Führe die folgenden Befehle einzeln aus (mit Enter bestätigen) und ersetze die "PLATZHALTER" mit den tatsächlichen Daten aus den Kinoklub Login-Daten:
    ...
    ```     
    setx ftp_user "ftp_user" 
    ```
    
    ```
    setx ftp_password "ftp_password"
    ```
...
2025 V3.04 Worklow Dateien upload Eintritte und Kiosk korrigiert.\
2026 V3.05 Speed up beim erstellen der Abrechnungen: Upload in parallel\
